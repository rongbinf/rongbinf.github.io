...
function hashNode(n) {
    return (0, base64_1.fromNumber)((0, cyrb53_1.cyrb53)(n.wholeText.trim()));
}
function countLeadingWhitespace(node) {
...
function selectionToFragment(selection) {
    ...
    for (var i = 0; i < selection.rangeCount; i++) {
        ...
        if (range && !range.collapsed) {
            ...
            if (startNode.nodeType == TEXT_NODE && endNode.nodeType == TEXT_NODE) {
                ranges.push([
                    [hashNode(startNode), startNode, (0, base64_1.fromNumber)(Math.max(range.startOffset - countLeadingWhitespace(startNode), 0))],
                    [hashNode(endNode), endNode, (0, base64_1.fromNumber)(Math.max(Math.min(range.endOffset - countLeadingWhitespace(endNode), endNode.wholeText.trim().length), 0))],
                    [[], 0, 0],
                ]);
            }
        }
    ...
    var fragmentParts = ranges.map(function (_a) {
        ...
        if (new Set(dupes).size != dupes.length) {
            ...
            fragmentPart += "~".concat(dupesString, "~").concat((0, base64_1.fromNumber)(startDupeOffset), "~").concat((0, base64_1.fromNumber)(endDupeOffset));
        }
        return fragmentPart;
    });
    return "#2".concat(fragmentParts.join());
}
...
function getRangeFromFragmentPart(fragmentPart) {
    ...
    _d = [startOffset, endOffset].map(base64_1.toNumber), startOffset = _d[0], endOffset = _d[1];
    ...
    if (dupeString && nodes.map(function (n) { return n[1] ? 's' : 'e'; }).join('') == dupeString) {
        startNode = nodes[(0, base64_1.toNumber)(dupeStartOffset)];
        endNode = nodes[(0, base64_1.toNumber)(dupeEndOffset)];
    }
